...
def randomize1(z, threshold, normal):
	...
	for y in range(width):
		for x in range(height):
			r = 128.0+127.0*  (snoise3(x*scale, y*scale, start + z*zScale, octaves=1, persistence=per))
			g = 128.0+127.0*  (snoise3(x*scale + dist, y*scale + dist, start + z*zScale, octaves=1, persistence=per))
			b = 128.0+127.0*  (snoise3(x*scale - dist, y*scale - dist, start + z*zScale, octaves=1, persistence=per))
			if(normal):
				curr[x][y][0] = r
				curr[x][y][1] = g
				curr[x][y][2] = b
			if r > 255*threshold or g > 255*threshold or b > 255*threshold:
				if normal:
					curr[x][y][3] = 1
					...
					if curr[x][y][3] == 0:
						curr[x][y][3] = 1
						controller.setPixel(x, y, curr[x][y][0], curr[x][y][1], curr[x][y][2])
					else:
						curr[x][y][3] = 0
	...
	for y in range(width):
		for x in range(height):
			...
			if len(neighbors) < 2 or len(neighbors) > 3:
				...
				if last[x][y][3] == 0:
					curr[x][y] = computeColor(neighbors)#last[120, 100, 500]#GETCOLOR
					controller.setPixel(x, y, curr[x][y][0], curr[x][y][1], curr[x][y][2])
				else:
					curr[x][y] = last[x][y]
					controller.setPixel(x, y, curr[x][y][0], curr[x][y][1], curr[x][y][2])
...
def difColor(c1, c2):
	# if int(c1[0]) == int(c2[0]) and int(c1[1]) == int(c2[1]) and int(c1[2]) == int(c2[2]):
	if c1[3] == c2[3]:
		return False
	return True
